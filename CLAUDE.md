...
Kotlin + Jetpack Compose + Material 3 原生刷题 App。设计文档：`docs/superpowers/specs/2026-07-17-shuati-android-design.md`。
...
4. 更新根目录 `version.json`（`version`/`update`/`notes` 同步），`git commit && git push origin master`
5. 签名沿用 `keystore/quiz.keystore`（pass/alias 在 `keystore.properties`）
